...
const settings = [
  ...
  {
    "DYNAMODB_TABLE_KEY": "java",
    "FETCH_API_ENDPOINT": "https://api.import.io/store/connector/0a570a00-9a70-4e73-88cf-9fd4f1009130/_query?input=webpage/url:http://tw.indeed.com/jobs?as_and=%26as_phr=%26as_any=%26as_not=%26as_ttl=java%26as_cmp=%26jt=fulltime%26st=%26radius=100%26l=%25E5%258F%25B0%25E5%258C%2597%25E5%25B8%2582%26fromage=last%26limit=10%26sort=date%26psf=advsrch&_apikey=c95c2cfe2b5648958728df00fbf5012babf6afe39896da33b497a0a12c3966bf560eba65d3aa7bd45f0f2ed54ee2b7e07510c6d963f1e7f77397de954f5f1b4c91dade04e30f271c1a1cc68779b1b543"
  }
]

export default async function (event, context) {
  try {
    // TODO: forEach 執行 async function
    await findJobs(settings[0])
    await findJobs(settings[1])
...
async function findJobs (setting) {
  ...
  const [
    ...
    fetchJobList(setting.FETCH_API_ENDPOINT),
    getLastJobAttributeByKey(setting.DYNAMODB_TABLE_KEY, 'LastUpdatedSourceID')
  ])
  ...
  // 根據 settings 的 key 從 function.json 取得 Slack 的 webhookUrl
  const webhookUrl = process.env[setting.DYNAMODB_TABLE_KEY]
  // 封裝一份 Slack 支援的接收格式
  ...
  const result = await Promise.all([
    updateJobAttributeByKey(setting.DYNAMODB_TABLE_KEY, { 'LastUpdatedSourceID': lastNewJobSourceID }),
    postNewJobsToSlackChannel(webhookUrl, attachments)
  ])

  console.log(result)
}

function fetchJobList (endpoint) {
...
function postNewJobsToSlackChannel (webhookUrl, attachments) {
  return postToChannel(webhookUrl, {
